-- set_attribute is decided by whether a value is present, not a separate
-- "present" flag.
--
-- An Attribute is always applicable to a Location of a valid type — there
-- is nothing to turn on or off, unlike a Service or Amenity. Only the
-- value is optional. The proposal payload dropped its `present` key
-- accordingly (src/lib/audits.ts, attributeDiff); this migration updates
-- the one place that read it.
  ...
  loop
    ...
    case p.kind
      ...
      when 'set_attribute' then
        -- Always applied here, never straight from a Finding: a capacity
        -- limit is worth a second look, unlike a service's working flag.
        -- An Attribute is always applicable to a valid type — there is no
        -- presence to decide, only whether a value is present (not null).
        if p.payload->'value' is not null and jsonb_typeof(p.payload->'value') <> 'null' then
          insert into location_attributes (location_id, attribute_id, value, note)
          values (loc, (p.payload->>'attribute_id')::uuid, (p.payload->>'value')::numeric, p.payload->>'note')
          on conflict (location_id, attribute_id) do update set value = excluded.value, note = excluded.note;
        else
