-- finalize_audit() applies set_attribute, the way it already applies
-- set_service and set_amenity. Kept in its own migration, after the one
-- that adds the enum value: a value ALTER TYPE ... ADD VALUE just added
-- cannot be referenced within the same transaction, and Supabase applies
